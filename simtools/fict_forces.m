...
% Get rotational planet data
planet = getPlanet();
angular_vel_radps = [0 0 getPlanet('angular_vel_radps')]; % need to size same as input...

% Calculate and sum fictitious forces
% (The Euler force is not included, because planetary rotation is constant)
coriolis_force_N = -2 .* object_mass_kg .* cross(angular_vel_radps, velocity_mps);
centrifugal_force_N = -cross(object_mass_kg .* angular_vel_radps, cross(angular_vel_radps, position_m));
