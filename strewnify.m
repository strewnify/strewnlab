...
projectile(n).force(current,3) = projectile(n).DragForce(current,3)-G_constant*planet.mass_kg*projectile(n).mass/(planet.ellipsoid_m.MeanRadius + projectile(n).position(current,3))^2; % z drag force and gravity applied down
...
%             end
        ...
        if projectile(n).inflight > 0
            ...
            if forcesplit
                ...
                projectile(rockcount).flighttime = 0;
                ...
                projectile(rockcount).force(current,3) = projectile(rockcount).DragForce(current,3)-G_constant*planet.mass_kg*projectile(rockcount).mass/(planet.ellipsoid_m.MeanRadius + projectile(rockcount).position(current,3))^2; % gravity applied down
...
for n = 1:rockcount
    ...
    if projectile(n).position(current,3) == plotlevel
        ...
        strewn_struct.impacttime = t(current);
