...
startlocation = reckon(ref_latitude, ref_longitude, ARCLEN, AZ,getPlanet('ellipsoid_m')); 
...
endlocation = reckon(ref_latitude, ref_longitude, ARCLEN, AZ,getPlanet('ellipsoid_m')); 
...
projectile(n).location(current,:) = reckon(ref_latitude, ref_longitude, ARCLEN, AZ,getPlanet('ellipsoid_m')); 
...
g = getConstant('G_constant')*getPlanet('mass_kg')/(getPlanet('ellipsoid_m').MeanRadius + projectile(n).position(current,3))^2;
...
projectile(n).force(current,3) = projectile(n).DragForce(current,3)-getConstant('G_constant')*getPlanet('mass_kg')*projectile(n).mass/(getPlanet('ellipsoid_m').MeanRadius + projectile(n).position(current,3))^2; % z drag force and gravity applied down
...
%                 [X, Y, Z] = geodetic2ecef(getPlanet('ellipsoid_m'), 47.359449, 2.279083, projectile(n).position(current,3));
...
%             % *** CORIOLIS ADDED *****  see line 565
            ...
            projectile(n).force(current,3) = projectile(n).DragForce(current,3) + projectile(n).MagnusForce(current,3) - getConstant('G_constant')*getPlanet('mass_kg')*projectile(n).mass/(getPlanet('ellipsoid_m').MeanRadius + projectile(n).position(current,3))^2; % gravity applied down
...
%             end
        ...
        if projectile(n).inflight > 0
            ...
            if forcesplit
                ...
                projectile(rockcount).location(current,:) = reckon(ref_latitude, ref_longitude, ARCLEN, AZ,getPlanet('ellipsoid_m')); 
                ...
                g = getConstant('G_constant')*getPlanet('mass_kg')/(getPlanet('ellipsoid_m').MeanRadius + projectile(rockcount).position(current,3))^2;
                ...
                projectile(rockcount).force(current,3) = projectile(rockcount).DragForce(current,3)-getConstant('G_constant')*getPlanet('mass_kg')*projectile(rockcount).mass/(getPlanet('ellipsoid_m').MeanRadius + projectile(rockcount).position(current,3))^2; % gravity applied down
