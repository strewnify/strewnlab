function [value] = getNEXRAD(parameter, mode)
    ...
    'VCP215', struct('name', 'VCP 215', 'elevations', [0.5, 0.9, 1.3, 1.8, 2.4, 3.1, 4, 5.1, 6.4, 8, 10, 12, 14, 16.7, 19.5], 'spin_rates_deg_s', [15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15], 'reflectivity', [true true true true true true true true true true true true true true true], 'mode_type', 'Precipitation'), ...
    ...
    switch lower(parameter)
        ...
        case 'vcp_duration_s'
            mode_needed = true;
            value =  numel(mode_data.(mode).spin_rates_deg_s) * 360 * (1/mean(mode_data.(mode).spin_rates_deg_s)) + VCP_gap_s;
