function [observations] = simsensor(entry_time, sim_time, projectile, sensor_db, station_list, observations)
...
for n = 1:size(projectile,2)
    if projectile(n).inflight > 0
        ...
        for i = 1:numel(station_list)
            ...
            new_rows = table( ...
                repmat(rockID, numrows), repmat(obs_time, numrows), repmat(sim_time, numrows), LAT * ones(numrows), ...
