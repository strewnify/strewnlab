...
if ~exist('AMS_data','var')
    ...
    if ~exist('AMS_data','var')
        InitCells = { 0 'text' 'text' 'text' 'text' 'text' NaT NaT 'text' 0 0 0 0 0 0 'text' 0 0 0 0 0 'text' 0 0 'text' 'text' 'text' 'text' 'text' 'text' 'text' 'text' 'text' 'text' 0 NaT 'text' 'text' 0 'text' 'text' 0 0 0 0 0 0 };
    ...
    for row = 1:numrows
        ...
        for column = 1:42
            ...
            switch variable_type
                case 'char'
                    % Datetime
                    if (column == 7) || (column == 8) || (column == 36)
                        try
                            eval(['AMS_data.' Variables{column} '(' num2str(startrow + row) ',' num2str(1) ') = datetime(AMS_raw{' num2str(row) '}.' Variables{column} ',''TimeZone'',''UTC'');'])
                            eval(['AMS_data.' Variables{column} '_Excel(' num2str(startrow + row) ',' num2str(1) ') = exceltime(datetime(AMS_raw{' num2str(row) '}.' Variables{column} ',''TimeZone'',''UTC''));'])
                        catch
...
for obs_i = 1:num_pending
    [AMS_data.lat_startproj, AMS_data.long_startproj] = reckon(AMS_data.latitude, AMS_data.longitude, linelength_km*1000, cellfun(@str2num,AMS_data.initial_azimuth), getPlanet('ellipsoid_m'));
